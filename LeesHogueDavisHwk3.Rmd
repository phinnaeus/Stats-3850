...
```{r}
spruce_t2 <- t.test(Spruce$Ht.change[Spruce$Fertilizer != "T"],Spruce$Ht.change[Spruce$Fertilizer != "F"], alternative="greater")
```
...
> SOLUTION: We are 95% confident that the proportion of men who voted for Bush from the 2002 General Social Survey is is within (`r prop.test(426,759)$conf.int[1]`, `r prop.test(426,759)$conf.int[2]`).  The intervals for the proportion of men who voted for Bush and the proportion of women who voted for Bush do not overlap; the male proportion is greater than the female proportion.  Thus we have sufficient statistical evidence to reject the null hypothesis that men and women vote for Bush with equal probability.
...
error <- 0.03
UCI <- 0.975
n <- ceiling((qnorm(UCI)/error)^2)
```

> SOLUTION: To provide a confidenced level of 95% and a margin of error of a maximum 3%, the marketers should get responses from a minimum `r n` people.
...
> SOLUTION: Due to the intervals overlapping, there is insufficient evidence to support the alternative hypothesis that the drug has an effect on hives break-out rates.
