...
> ANSWER: The bootstrapped mean, $\bar{X}^*$, is `r mean(boot.mean)` and standard error, $\hat{\sigma}^*_{\bar{X}}$, is `r sd(boot.mean)`
...
> ANSWER:  The bootstrapped mean, $\bar{X}^*$, is `r mean(boot.mean)` and standard error, $\hat{\sigma}^*_{\bar{X}}$, is `r sd(boot.mean)`. The bootstrap distribution has a bias of `r mean(boot.mean)-mean(chlorine)`.
...
plot2 <- q + geom_boxplot() + coord_flip()#+ xlim(0,2)
