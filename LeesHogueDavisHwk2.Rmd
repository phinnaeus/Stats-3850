...
randtest.paired <- function(x, y, fun = mean, reps = 10000 - 1, COL = "lightblue"){ 
  ...
  curve(dnorm(x, mean(results), sd = sd(results)), add = TRUE, col = "blue")
...
p + geom_histogram(aes(y = ..density..), fill = "lightskyblue")+ geom_density(color = "brown1", size = 1.5)
...
############### HELP ###############
# Should I report the standar error as $\sigma^*$
####################################
```
...
chlorineplot + geom_histogram(aes(y = ..density..), fill = "blue")+ geom_density(color = "red", size = 1.5)
...
plot1 <- p + geom_histogram(aes(y = ..density..), fill = "blue")+ geom_density(color = "red", size = 1.5)
plot2 <- q + geom_boxplot() + coord_flip()

multiplot(plot2,plot1,cols=1)

############### HELP ###############
# Can i use ggplot to line up the scale bars
# and what is going on with the x axis in the 
# bar plot?!?!
####################################
```
...
############### HELP ###############
# knitting in scientific notation!?!?
####################################
```
...
p1 <- ggplot(data = df, aes(x = Calories)) + geom_histogram(aes(y = ..density..), fill = "blue") + facet_grid(Flavor ~ .) + geom_density(color = "red", size = 1.5)
p1
...
############### HELP ###############
# Should I report for my CI, and is 
# my CI too big?!?!
####################################
```
...
p1 <- ggplot(data = DUAAA, aes(x = Delay)) + geom_histogram(aes(y = ..density..), fill = "blue") + facet_grid(Carrier ~ .) + geom_density(color = "red", size = 1.5)
p1
...
p <- ggplot(as.data.frame(boot.ratio),aes(x=boot.ratio),color="red")
p + geom_histogram(aes(y = ..density..), fill = "blue")+ geom_density(color = "red", size = 1.5)
...
############### HELP ###############
# what does What is the bootstrap estimate of the bias? 
# and What fraction of the bootstrap standard error does it represent? mean
####################################
```
...
```{r message=FALSE}
p <- ggplot(as.data.frame(twins$Difference),aes(x=twins$Difference),color="red")
p + geom_histogram(aes(y = ..density..), fill = "blue")+ geom_density(color = "red", size = 1.5)
...
```{r}
result <- randtest.paired(twins$Unaffected, twins$Affected, COL = "green", reps = 10000-1)
```
